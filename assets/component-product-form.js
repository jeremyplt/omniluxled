...
class ProductForm extends HTMLElement {
  constructor() {
    ...
    this.qtyBtns.forEach((qtyBtn) =>
      qtyBtn.addEventListener("click", this.manageQtyBtn.bind(this))
    );
  }
  ...
  onSubmitHandler(evt) {
    ...
    if (pdpContainer) {
      let addOnItemsEle = pdpContainer.querySelectorAll(
        "[data-addon-selection]:checked"
      );
    ...
    fetch(`${routes.cart_add_url}`, { ...fetchConfig(), body })
      ...
      .then(() => {
        if (document.querySelector("#PopupModal-quickshop")) {
          document
            .querySelector("#PopupModal-quickshop .close-quickshop")
...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange(_event) {
    ...
    if (this.formType == "product-page")
      window.globalVariables.product.currentVariant = this.currentVariant;
  ...
  _getOptionsFromRadio() {
    const fieldsets = Array.from(
      this.querySelectorAll("fieldset:not(.addon-fieldset)")
    );
    return fieldsets.map((fieldset) => {
      return Array.from(
        fieldset.querySelectorAll("input:not(.addon-swatch)")
  ...
  setCurrentVariant() {
    ...
    let options =
      this.variantPicker == "variant-select"
        ? this._getOptionsFromSelect()
        : this._getOptionsFromRadio();
  ...
  updateURLandID(currentVariant) {
    ...
    if (this.formType == "product-page")
      window.history.replaceState(
        {},
        "",
        `${this.dataset.url}?variant=${currentVariant.id}`
      );
  ...
  renderProductInfo(currentVariant, container) {
    ...
    // Price Update
    let price = Shopify.formatMoney(
      this.calculatePriceWithTaxes(currentVariant.price, customerCountry),
      window.globalVariables.money_format
    );
    let compare_price = Shopify.formatMoney(
    ...
    if (this.formType == "grid") {
      ...
      let imageSlide = document.querySelector(
        `.product--media[data-mediaID="${imageID}"]`
      );
      if (this.formType == "quickshop") {
        imageSlide = document.querySelector(
          `.quickshop-slider .product--media[data-mediaID="${imageID}"]`
        );
      }
      if (!imageSlide) return;
      let slideIndex = Array.from(imageSlide.parentNode.children).indexOf(
        imageSlide
      );
  ...
  _getVariantData() {
    this.variantData =
      this.variantData || JSON.parse(this.variant_json.textContent);
  ...
  constructor() {
    ...
    if (colorSwatchContainer) {
      ...
      colorSwatches.forEach((swatch) => {
        let colorHandle = swatch.querySelector('input[type="radio"]').dataset
          .handle;
        let swatchStyle = Utility.getSwatchStyle(colorHandle);
        swatch
          .querySelector(".swatch-label")
          .setAttribute("style", swatchStyle);
      });
    }
  }
}
